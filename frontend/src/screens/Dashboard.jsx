...
import { Target, Clock, TrendingUp, Flame, Sparkles, ArrowRight } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const reload = () => {
    ...
    taskApi.list({ from: today.toISOString(), to: tomorrow.toISOString() }).then(setTasks);
  ...
  return (
    <div className="fade-up" style={{ display: 'flex', flexDirection: 'column', gap: 40 }}>
      ...
      <section>
        ...
        <h1 style={{ marginTop: 12, fontFamily: fontDisplay, fontSize: '3.5rem', fontWeight: 400, lineHeight: 1.05, letterSpacing: '-0.025em' }}>
          {greet}, <em style={{ color: c.forestL }}>{firstName}.</em>
        </h1>
        <p style={{ marginTop: 12, maxWidth: 620, color: c.muted, fontSize: '1.05rem' }}>
          VocÍ tem <strong style={{ color: c.ink }}>{tasks.length - done} tarefas</strong> pendentes hoje,
          somando <strong style={{ color: c.ink }}>{Math.round(totalMin/60*10)/10}h</strong> de trabalho previsto.
        </p>
      </section>

      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
        <Stat Icon={Target}     title="Tarefas hoje"    value={`${done}/${tasks.length}`} sub="concluÌdas" tint={c.forest}/>
        <Stat Icon={Clock}      title="Tempo previsto" value={`${Math.floor(totalMin/60)}h${totalMin%60}m`} sub="hoje" tint={c.gold}/>
        <Stat Icon={TrendingUp} title="Precis„o"       value={`${overview?.precision_pct ?? '?'}%`} sub="estimativa vs real" tint={c.sage}/>
        <Stat Icon={Flame}      title="SequÍncia"      value={`${stats?.current_streak ?? 0} d`} sub={`recorde: ${stats?.longest_streak ?? 0}`} tint={c.rust}/>
      </section>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 24 }}>
        <div style={{ background: c.paper, border: `1px solid ${c.borderS}`, borderRadius: 24, padding: 28 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            <h2 style={{ fontFamily: fontDisplay, fontSize: '1.5rem', fontWeight: 500, margin: 0 }}>Agenda de hoje</h2>
            <span style={{ fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', color: c.muted }}>{tasks.length} itens</span>
          </div>
          <div style={{ marginTop: 16 }}>
            {tasks.length === 0
              ? <div style={{ textAlign: 'center', padding: 48, color: c.muted }}><Sparkles size={28}/><div style={{ marginTop: 8 }}>Nada agendado para hoje.</div></div>
              : tasks.map(t => <TaskRow key={t.id} task={t} onChange={reload}/>)}
          </div>
        </div>
          ...
          {suggestions.length > 0 && (
            <div style={{ background: c.forest, color: c.creamL, borderRadius: 24, padding: 28 }}>
              ...
              <div style={{ fontSize: 12, color: 'rgba(245,239,227,0.65)', marginTop: 8 }}>
                Baseado em {suggestions[0].samples} tarefas concluÌ≠das.
              </div>
            </div>
          )}

          <div style={{ background: c.paper, border: `1px solid ${c.borderS}`, borderRadius: 24, padding: 24 }}>
            <h3 style={{ fontFamily: fontDisplay, fontSize: '1.15rem', fontWeight: 500, margin: 0 }}>Carga semanal</h3>
            <div style={{ fontSize: 12, color: c.muted, marginTop: 4 }}>
              {overview?.workload?.level && (
                <span style={{ textTransform: 'capitalize' }}>‚óè {overview.workload.level}</span>
              )}
            </div>
            <div style={{ marginTop: 12, fontFamily: fontDisplay, fontSize: '2rem' }}>
              {overview?.workload?.pending_hours ?? '‚Äî'}h
            </div>
            <div style={{ fontSize: 11, color: c.muted }}>{overview?.workload?.message}</div>
          </div>
        </div>
      </section>
      ...
      {overview?.weekly && (
        <section style={{ background: c.paper, border: `1px solid ${c.borderS}`, borderRadius: 24, padding: 28 }}>
          <h2 style={{ fontFamily: fontDisplay, fontSize: '1.5rem', fontWeight: 500, margin: '0 0 24px' }}>
            Esta semana È previsto vs real
          </h2>
...
const Stat = ({ Icon, title, value, sub, tint }) => (
  <div style={{ background: c.paper, border: `1px solid ${c.borderS}`, borderRadius: 18, padding: 20 }}>
    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
      <span style={{ fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: c.muted }}>{title}</span>
      <Icon size={16} color={tint}/>
    </div>
    <div style={{ marginTop: 12, fontFamily: fontDisplay, fontSize: '2rem', lineHeight: 1, color: c.ink }}>{value}</div>
