...
export const Logo = ({ size = 28, color = c.forest }) => (
  <svg width={size} height={size} viewBox="0 0 40 40" fill="none">
    ...
    <path d="M12 20 L18 26 L29 14" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
...
export const Brand = ({ size = 'md', color = c.forest }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: fontDisplay }}>
      <Logo size={size === 'lg' ? 32 : 24} color={color} />
      <span style={{ fontSize: sizes[size], color, fontWeight: 500, letterSpacing: '-0.02em' }}>
        Planly
      </span>
    </div>
  );
